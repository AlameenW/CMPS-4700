...
Cleans + encodes raw dataset into ML-ready numeric form
"""
import pandas as pd
...
def preprocess(df):
    ...
    """ 
    Cleans raw dataset, encodes categorical variables,
    and prepares data for feature generation
    """
    ...
    # Missing value (Q Columns)
    existing_score_columns = [c for c in score_columns if c in df.columns]

    df[existing_score_columns] = df[existing_score_columns].fillna(
        df[existing_score_columns].median()
    )
    # #

    # Feature creation (MH Score)
    df['MH_Score'] = df[existing_score_columns].sum(axis=1)
    # # 
    ...
    df['TARGET'] = df['MH_Score'].apply(assign_risk)
    # #
    ...
    # One-hot encoding
    categorical_columns = ['Gender', 'Relationship_Status', 'Occupation', 'Organization']
    df = pd.get_dummies(
        df,
